...
public class StrutsFrameworkInitializer implements ProjectActivity {
    ...
    public Object execute(@NotNull Project project, @NotNull Continuation<? super Unit> continuation) {
        ...
        InitializationData data = pendingInitializations.remove(project.getName());
        if (data != null) {
     ...
     * <p>
     * This method checks if JAR files are already present in existing file sets to avoid duplicates.
     * This can happen when the facet was created via framework detection (StrutsFrameworkDetector),
        ...
        DumbService.getInstance(data.project).runWhenSmart(() -> {
            try {
                ...
                if (!jarConfigFiles.isEmpty()) {
                    // Check if JAR files are already present in existing file sets
                    final Set<StrutsFileSet> existingFileSets = strutsFacetConfiguration.getFileSets();
                    
                    // Collect JAR files that are not already in any existing file set
                    ...
                    for (final VirtualFile jarFile : jarConfigFiles.keySet()) {
                        ...
                        for (final PsiFile configFile : configFiles) {
                            ...
                            if (virtualFile != null) {
                                // Check if this file is already in any existing file set
                    ...
                    // Only create a new file set if there are JAR files not already present
                    if (!newJarFiles.isEmpty()) {
                        ...
                        int skippedCount = totalJarFilesCount - newJarFiles.size();
                        LOG.info("Added " + newJarFiles.size() + " new JAR config files to Struts facet configuration " +
                                 (skippedCount > 0 ? "(skipped " + skippedCount + " already present)" : ""));

                        // Show success notification after JAR discovery
                        showSuccessNotification(data.project, data.strutsFacet);
                    } else {
                        LOG.info("All " + totalJarFilesCount + " JAR configuration files are already present in existing file sets, skipping duplicate creation");
                        // Still show success notification even if no new files to add
                        showSuccessNotification(data.project, data.strutsFacet);
                    }
                } else {
