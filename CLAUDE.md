...
The base value `18969` maintains historical continuity from when the plugin was donated by JetBrains to Apache Software Foundation, ensuring version numbers continue from the previous build sequence.
...
**Test Data Path Resolution**: IntelliJ 2024.2+ changed how `LexerTestCase` and `DomStubTest` resolve paths. If tests fail with "Cannot find source file", override path methods to use project-relative paths.
...
**JSP Reference Providers**: The `javaee.web.customServletReferenceProvider` extension point behavior changed in 2024.2. Action link references in JSP may need alternative registration approaches.
